...
def memory_distill(
    ...
    return get_service().distill_session(session_id=session_id, max_lines=max_lines, namespace=namespace)


@mcp.tool(name="memory.search")
def memory_search(query: str, k: int = 5, namespace: str | None = None) -> list[dict[str, Any]]:
    """Search memory notes using configured embeddings backend."""
    return get_service().memory_search(query=query, k=k, namespace=namespace)


@mcp.tool(name="policy.get")
def policy_get(active_version: bool = True, namespace: str | None = None) -> dict[str, Any]:
    """Get active policy content and metadata."""
    _ = active_version
    return get_service().policy_get(namespace=namespace)
...
def policy_propose(
    ...
    return get_service().policy_propose(delta_md=delta_md, evidence_refs=evidence_refs, namespace=namespace)


@mcp.tool(name="policy.evaluate")
def policy_evaluate(proposal_id: str, namespace: str | None = None) -> dict[str, Any]:
    """Run weighted gate checks and regression suite for a policy proposal."""
    return get_service().policy_evaluate(proposal_id=proposal_id, namespace=namespace)


@mcp.tool(name="policy.promote")
def policy_promote(proposal_id: str, namespace: str | None = None) -> dict[str, Any]:
    """Promote a passing policy proposal to active policy."""
    return get_service().policy_promote(proposal_id=proposal_id, namespace=namespace)


@mcp.tool(name="policy.rollback")
def policy_rollback(version_id: str, namespace: str | None = None) -> dict[str, Any]:
    """Rollback active policy to a previously promoted version."""
    return get_service().policy_rollback(version_id=version_id, namespace=namespace)
